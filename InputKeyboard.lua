--[[
	...
	USAGE:
		Create new keyboard: KeyBoard.new(language)
...
function InputBox:onKeyUp(event)
...
function InputBox:onKeyDown(event)

if event.keyCode==0 then return end

if not self.keyboard then
--  print (event.keyCode)
  
  if event.keyCode==KeyCode.BACKSPACE then  -- BACKSPACE
  
    if self.textContent:len()>0 then
      local lastByte = string.byte(string.sub(self.textContent,self.textContent:len(),self.textContent:len()))
      local delLength = 1
      if lastByte and lastByte >=128 and lastByte <=192 then
	    delLength = 2	-- The character to remove uses two bytes in the string
      end
		
      self.textField:setText(string.sub(self.textField:getText(),1,self.textField:getText():len()-delLength))
      self.textContent = string.sub(self.textContent,1,self.textContent:len()-delLength)
	end
	
  elseif event.keyCode==KeyCode.SHIFT then  -- SHIFT
  
    self.shiftHeld=true
	
  elseif event.keyCode==KeyCode.SPACE then   -- SPACE

    self.textField:setText(self.textField:getText().." ")
    self.textContent = self.textContent.." "
  
  else  -- letter or number
    local letter=string.char(event.keyCode)
	if not self.shiftHeld then
	   letter=letter:lower()
	end
	   
    self.textField:setText(self.textField:getText()..letter)
    self.textContent = self.textContent..letter
  end
end
